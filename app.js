...
/* 20 COLORES PARA EL MARCO EXTERIOR */
const MARCOS = [
  ...
  {c:'#b76e79',n:'Rosa viejo'},{c:'#6c4f3c',n:'Marón'},{c:'#3a6b4b',n:'Verde esmeralda'},
  {c:'#8b3a3a',n:'Ladrillo'},{c:'#2c4c6b',n:'Azul acero'}
];
...
function startAdd() {
  editIdx = -1;
  document.getElementById('msgTA').value = '';
  document.getElementById('msgEditor').classList.add('open');
  document.getElementById('msgTA').focus();
}
function startEdit(e, i) {
  e.stopPropagation();
  editIdx = i;
  document.getElementById('msgTA').value = MSGS[i];
  document.getElementById('msgEditor').classList.add('open');
  document.getElementById('msgTA').focus();
}
function cancelEdit() {
  document.getElementById('msgEditor').classList.remove('open');
  editIdx = -1;
}
function saveEdit() {
...
function delMsg(e, i) {
  ...
  MSGS.splice(i, 1);
  if (selMsg >= MSGS.length) selMsg = MSGS.length - 1;
  buildMsgList();
  render();
}
...
function buildPal(id, items, getActive, onSel, columnsClass='pal5') {
  const el = document.getElementById(id);
  el.innerHTML = '';
  el.className = columnsClass;
  items.forEach(p => {
    const s = document.createElement('div');
    const active = (p === getActive());
    s.className = 'sw' + (active ? ' sel' : '');
    s.style.background = p.c;
    s.style.color = isLight(p.c) ? '#222' : '#fff';
    if (p.c === '#ffffff') s.style.border = '1.5px solid #ccc';
    s.title = p.n;
    if (active) s.textContent = '✓';
    s.onclick = () => {
      document.querySelectorAll('#' + id + ' .sw').forEach(x => {
        x.classList.remove('sel'); x.textContent = '';
        if (x.style.backgroundColor === 'rgb(255, 255, 255)') x.style.border = '1.5px solid #ccc';
      });
      s.classList.add('sel'); s.textContent = '✓';
      onSel(p); render();
    };
    el.appendChild(s);
  });
...
function buildTxtPal() {
  ...
  TEXTOS.forEach(p => {
    ...
    ch.onclick = () => {
      document.querySelectorAll('#palTexto .tcc').forEach(x => x.classList.remove('sel'));
...
function initAlignToggle() {
  const opts = document.querySelectorAll('.align-opt');
  opts.forEach(opt => {
    opt.onclick = () => {
      opts.forEach(o => o.classList.remove('sel', 'active'));
      opt.classList.add('sel', 'active');
      textAlign = opt.getAttribute('data-align');
      render();
    };
  });
}
...
function initSliders() {
  const marcoSlider = document.getElementById('marcoSizeSlider');
  const marcoValue = document.getElementById('marcoSizeValue');
  const fotoSlider = document.getElementById('fotoSizeSlider');
  const fotoValue = document.getElementById('fotoSizeValue');

  marcoSlider.addEventListener('input', (e) => {
    marcoSize = parseInt(e.target.value);
    marcoValue.textContent = marcoSize + 'px';
    render();
  });

  fotoSlider.addEventListener('input', (e) => {
    fotoSize = parseInt(e.target.value);
    const labels = ['Muy pequeña','Pequeña','Mediana-pequeña','Mediana','Mediana-grande','Grande','Muy grande','Extra grande','Máxima'];
    const idx = Math.floor(fotoSize / 12.5);
    fotoValue.textContent = labels[Math.min(idx, labels.length - 1)];
    render();
  });
}

/* ── FUENTE / LAYOUT ── */
function setFont(el, fam, gen, wt) {
  document.querySelectorAll('.fo').forEach(x => x.classList.remove('active'));
  el.classList.add('active');
  fontFam = fam; fontGen = gen; fontWt = wt;
  render();
}
function setLayout(el, l) {
...
/* ── UPLOAD ── */
function onUpzClick(e) {
  if (e.target.classList.contains('upz-chg')) return;
...
function resetImg(e) {
  ...
  z.innerHTML = `<div class="upz-ico">🖼️</div><div class="upz-lbl"><strong>Clic para subir</strong><br>JPG · PNG · WEBP</div><div class="upz-chg" onclick="resetImg(event)">✕ Cambiar</div>`;
...
document.getElementById('fi').addEventListener('change', e => {
  ...
  r.onload = ev => {
    ...
    const z = document.getElementById('upz');
    z.classList.add('has');
    z.innerHTML = `<img src="${imgSrc}" alt=""><div class="upz-chg" onclick="resetImg(event)">✕ Cambiar</div>`;
    render();
  };
  r.readAsDataURL(f);
});
...
function render() {
  ...
  let innerHTML = '';
  if (!isLR) {
    const photoBlock = `<div class="cptb" style="padding:${photoPadding}px;">
      <div class="img-frame" style="height:${photoHeight}px;">${photoContent}</div>
    </div>`;
    const textBlock = `<div class="cth" style="background:${fondo.c};padding:${textPadding}px 20px;">
      <div class="${alignClass}" style="${fstyle}">${msg}</div>
    </div>`;
    innerHTML = layout === 'top' ? textBlock + photoBlock : photoBlock + textBlock;
  } else {
    const photoBlock = `<div class="cplr" style="width:${photoWidthPercent}%;padding:${photoPaddingLR}px;">
      <div class="img-frame" style="aspect-ratio:1/0.9;">${photoContent}</div>
    </div>`;
    const textBlock = `<div class="cthlr" style="background:${fondo.c};padding:${textPadding}px 16px;">
      <div class="${alignClass}" style="${fstyle}">${msg}</div>
    </div>`;
  ...
  document.getElementById('cardWrap').innerHTML = `
    <div class="card-outer" style="border-color:${marco.c};border-width:${marcoSize}px;border-style:solid;">
      <div class="card-inner" style="background:${fondo.c};width:${W}px;">
        ${innerHTML}
      </div>
    </div>`;
}
...
buildPal('palFondo', FONDOS, () => fondo, p => { fondo = p; }, 'pal5');
buildTxtPal();
initAlignToggle();
initSliders();
render();
